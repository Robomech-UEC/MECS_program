...
などがあります。  
この通信方式はI2CやSPIと異なり、物理層までの規格しか存在せず、RS-485を用いた様々な通信規格が存在します。([制御における通信](../../../004_control/contents/100_implementation/001_communication/index)参照)  

- 半二重、全二重
I2Cのように情報の入出力で共通の通信線を用いるものを半二重通信、SPIのように別の線を用いて通信を行うものを全二重通信と呼びます。  
- one-wire
通常、RS-485はひとつの通信に対して2本の線が用いられますが、これを1本で行っているものを特にone-wire通信と呼びます。

物理層  
ここでは、半二重通信の内one-wireでないものの物理層について説明します。  
